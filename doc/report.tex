...
En el diagrama anterior, queremos calcular la profundidad (\Scalar{z}) a partir de la disparidad, es decir, la distancia entre los puntos en el plano de la imagen, correspondiendo al punto 3D \ThreeCart{M} de la escena:

\[
    disparidad = B - (\ThreeCart{m_1} - \ThreeCart{m_2})
\]

donde \Scalar{B} es la apertura horizontal de la cámara estéreo (distancia entre los centros de las dos cámaras).

Dado que los triángulos $\ThreeCart{M}\ThreeCart{m_1}\ThreeCart{m_2}$ y $\ThreeCart{M}\ThreeCart{C_1}\ThreeCart{C_2}$ son semejantes, podemos plantear:

\begin{align*}
    \frac{\ThreeCart{m_1} - \ThreeCart{m_2}}{B} & = \frac{z - f}{z} \\
    \ThreeCart{m_1} - \ThreeCart{m_2} & = B - \frac{Bf}{z} \\
    B - (\ThreeCart{m_1} - \ThreeCart{m_2}) & = \frac{Bf}{z} \\
